 ...
 * thread.
 * <p>
 * When the query thread crashes, the locks it used can no longer be unlocked by itself and another
 * thread must do the unlock for it. Unfortunately, normal Java ReentrantLock can only be unlocked
 * by the thread which locked it, so we have to use a different implementation. It is also
