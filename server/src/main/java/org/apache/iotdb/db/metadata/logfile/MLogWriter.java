...
public class MLogWriter implements AutoCloseable {
  ...
  private void sync() {
    try {
      ...
      logger
          .error("MLog {} sync failed, change system mode to read-only", logFile.getAbsoluteFile(),
              e);
  ...
  public void deleteStorageGroup(PartialPath storageGroup) throws IOException {
    DeleteStorageGroupPlan plan = new DeleteStorageGroupPlan(
  ...
  public void serializeStorageGroupMNode(StorageGroupMNode node) throws IOException {
    ...
    StorageGroupMNodePlan plan = new StorageGroupMNodePlan(node.getName(), node.getDataTTL(),
        childSize);
    putLog(plan);
  }
  ...
  public static void upgradeMLog() throws IOException {
    ...
    upgradeTxtToBin(schemaDir, MetadataConstant.METADATA_TXT_LOG, MetadataConstant.METADATA_LOG,
        false);
    upgradeTxtToBin(schemaDir, MetadataConstant.MTREE_TXT_SNAPSHOT, MetadataConstant.MTREE_SNAPSHOT,
        true);
  }
    ...
    switch (args[0]) {
      case MetadataOperationType.CREATE_TIMESERIES:
        ...
        CreateTimeSeriesPlan plan = new CreateTimeSeriesPlan(new PartialPath(args[1]),
            ...
            CompressionType.deserialize((byte) Short.parseShort(args[4])), props, null, null,
            alias);
  ...
  public static PhysicalPlan convertFromString(String str) {
    ...
    switch (words[0]) {
      case "2":
        return new MeasurementMNodePlan(words[1], words[2].equals("") ? null : words[2],
      ...
      case "1":
        return new StorageGroupMNodePlan(words[1], Long.parseLong(words[2]),
