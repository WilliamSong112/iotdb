 ...
 * <p>
 * { "device":"root.sg.d1", "timestamps":[1586076045524,1586076065526], "measurements":["s1","s2"],
...
public class JSONPayloadFormatter implements PayloadFormatter {
  ...
  public List<Message> format(ByteBuf payload) {
    ...
    for (int i = 0; i < timestamps.size(); i++) {
      ...
      message.setMeasurements(GSON.fromJson(measurements, new TypeToken<List<String>>() {
      }.getType()));
      message.setValues(GSON.fromJson(values.get(i), new TypeToken<List<String>>() {
      }.getType()));
      ret.add(message);
    }
    return ret;
